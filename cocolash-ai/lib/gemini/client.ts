 ...
 * selfie.
 *
 * Live-tested 2026-06-15: ALL gemini-3 image models — gemini-3-pro-image,
 * gemini-3-pro-image-preview, and gemini-3.1-flash-image — refuse to edit a real
 * face and return promptFeedback.blockReason="OTHER" (Google's likeness/
 * real-person RAI filter). This is NOT fixable via safetySettings OR by
 * reframing the prompt — we tried both. `gemini-2.5-flash-image` (nano-banana)
 * is the GA image-EDITING model that permits real-selfie edits and the one the
 * try-on shipped/demoed on. Keep this as the try-on model. Overridable via env.
 */
